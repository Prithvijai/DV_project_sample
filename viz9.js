document.addEventListener("DOMContentLoaded", () => {
  ...
  function animateFill(wave, lifePercent, duration = 1500, color = "#00ff99") {
    ...
    function step(now) {
      ...
      const waveAmp = 10;
      const waveFreq = 800;
      const offset = Math.sin(now / waveFreq) * 20;

      const d = `
      M0,${baseY}
      Q150,${baseY - waveAmp + Math.sin(now / 500) * 5} 300,${baseY + offset / 4}
      T550,${baseY}
      L550,950 L0,950 Z
    `;
      ...
      if (t < 1) {
        ...
        // Continuous gentle ripple motion when settled
        requestAnimationFrame(function ripple(ts) {
          const offset = Math.sin(ts / 1200) * 12; // Slower, calmer ripple
          const d = `
          M0,${targetY}
          Q150,${targetY - waveAmp + Math.sin(ts / 800) * 4} 300,${targetY + offset / 4}
          T550,${targetY}
          L550,950 L0,950 Z
        `;
          wave.setAttribute("d", d);
          // Keep rippling but check if we need to stop? 
          // For this viz, continuous subtle movement is "alive" and professional.
